...
static int dcss_crtc_init(struct dcss_crtc *dcss_crtc,
			  ...
			  struct drm_device *drm)
{
	...
	dcss_crtc->plane[0] = dcss_plane_init(drm, dcss, drm_crtc_mask(crtc),
					 DRM_PLANE_TYPE_PRIMARY, 2);
	if (IS_ERR(dcss_crtc->plane[0]))
		return PTR_ERR(dcss_crtc->plane[0]);

	crtc->port = pdata->of_node;
	drm_crtc_helper_add(crtc, &dcss_helper_funcs);
	ret = drm_crtc_init_with_planes(drm, crtc, &dcss_crtc->plane[0]->base,
					NULL, &dcss_crtc_funcs, NULL);
	if (ret) {
		dev_err(dcss_crtc->dev, "failed to init dcss_crtc\n");
		return ret;
	}
#if 0
	dcss_crtc->plane[1] = dcss_plane_init(drm, dcss, drm_crtc_mask(crtc),
					 DRM_PLANE_TYPE_OVERLAY, 1);
	if (IS_ERR(dcss_crtc->plane[1]))
		dcss_crtc->plane[1] = NULL;

	dcss_crtc->plane[2] = dcss_plane_init(drm, dcss, drm_crtc_mask(crtc),
					 DRM_PLANE_TYPE_OVERLAY, 0);
	if (IS_ERR(dcss_crtc->plane[2]))
		dcss_crtc->plane[2] = NULL;
#endif
	dcss_crtc->alpha = drm_property_create_range(drm, 0, "alpha", 0, 255);
	if (!dcss_crtc->alpha) {
		dev_err(dcss_crtc->dev, "cannot create alpha property\n");
		return -ENOMEM;
	}

	dcss_crtc->use_global = drm_property_create_range(drm, 0,
						     "use_global_alpha", 0, 1);
	if (!dcss_crtc->use_global) {
		dev_err(dcss_crtc->dev, "cannot create use_global property\n");
		return -ENOMEM;
	}

	/* attach alpha property to channel 0 */
	drm_object_attach_property(&dcss_crtc->plane[0]->base.base,
				   dcss_crtc->alpha, 255);
	dcss_crtc->plane[0]->alpha_prop = dcss_crtc->alpha;

	drm_object_attach_property(&dcss_crtc->plane[0]->base.base,
				   dcss_crtc->use_global, 0);
	dcss_crtc->plane[0]->use_global_prop = dcss_crtc->use_global;

	dcss_crtc->irq = dcss_vblank_irq_get(dcss);
	if (dcss_crtc->irq < 0) {
		dev_err(dcss_crtc->dev, "unable to get vblank interrupt\n");
		return dcss_crtc->irq;
	}

	init_completion(&dcss_crtc->disable_completion);

	ret = devm_request_irq(dcss_crtc->dev, dcss_crtc->irq, dcss_crtc_irq_handler,
			       0, "dcss_drm", dcss_crtc);
	if (ret) {
		dev_err(dcss_crtc->dev, "irq request failed with %d.\n", ret);
		return ret;
	}

	disable_irq(dcss_crtc->irq);
