...
void imx8mq_phy_reset(state_struct *state, struct hdp_mem *mem, u8 reset)
{
...
static int imx_hdp_connector_get_modes(struct drm_connector *connector)
{
	...
	if (!hdp->no_edid) {
		...
		if (edid) {
			...
			dev_warn(hdp->dev, "failed to get edid, use default video modes\n");
...
//	struct hdr_static_metadata *hdr_metadata;
	...
	if (1 /*!hdp->ops->write_hdr_metadata*/)
		goto out;
#if 0
			    ...
			    void *data)
{
	...
	imx_hdp_call(hdp, phy_reset, &hdp->state, &hdp->mem, 0);
