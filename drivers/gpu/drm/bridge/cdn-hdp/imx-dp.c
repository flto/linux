		...
		int color_depth)
{
	...
	/* reset phy */
	imx_hdp_call(hdp, phy_reset, &hdp->state, NULL, 0);
	...
	imx_hdp_call(hdp, phy_reset, &hdp->state, NULL, 1);
		...
		struct drm_dp_aux_msg *msg)
{
	...
	DRM_DEBUG("\n");
	DRM_DEBUG("%s() msg->request 0x%x msg->size 0x%x\n",
			...
			int max_link_rate)
{
	...
	DRM_DEBUG("dp_mode_set()\n");

	drm_dp_downstream_id(&hdp->aux, linkid);
	DRM_DEBUG("DP link id: %s, 0x%x 0x%x 0x%x 0x%x 0x%x 0x%x\n",
		 linkid, linkid[0], linkid[1], linkid[2], linkid[3], linkid[4],
		 linkid[5]);

	drm_dp_link_probe(&hdp->aux, &link);
	DRM_DEBUG("DP revision: 0x%x\n", link.revision);
	DRM_DEBUG("DP rate: %d Mbps\n", link.rate/100);
	DRM_DEBUG("DP number of lanes: %d\n", link.num_lanes);
	DRM_DEBUG("DP capabilities: 0x%lx\n", link.capabilities);
	...
	DRM_DEBUG("CDN_API_DPTX_SetHostCap_blocking (ret = %d)\n", ret);
	...
	DRM_DEBUG("CDN_API_DPTX_Set_VIC_blocking (ret = %d)\n", ret);
	...
	do {
		...
		if (dp_check_link_status(state, num_lanes) == true) {
		        DRM_DEBUG("Link is good - Training complete\n");
		        break;
		} else {
			DRM_DEBUG("Link is bad - need to restart training\n");
			training_restarts--;
			training_retries = 20;
	...
	DRM_DEBUG("dp_check_link_status %d\n", dp_check_link_status(state, num_lanes));

	/* Set video on */
	ret = CDN_API_DPTX_SetVideo_blocking(state, 1);
	DRM_DEBUG("CDN_API_DPTX_SetVideo_blocking (ret = %d)\n", ret);
...
int dp_get_edid_block(void *data, u8 *buf, unsigned int block, size_t len)
{
	...
	DRM_DEBUG("dp_get_edid_block (ret = %d) block %d\n", ret, block);
	if (ret == CDN_OK) {
		       ...
		       int color_depth)
{
	...
	DRM_DEBUG("asserted HDP PHY reset\n");

	dp_phy_pix_engine_reset_t28hpc(state);
	DRM_DEBUG("pixel engine reset\n");
	...
	DRM_DEBUG("CDN_*_Write_Register_blocking ... setting LANES_CONFIG %x\n",
		 lane_mapping);
