		...
		struct dpu_hw_intf_cfg *cfg)
{
	...
	DPU_REG_WRITE(c, CTL_MERGE_3D_ACTIVE, cfg->mode_3d ? 1 : 0);
}
...
static inline int dpu_hw_ctl_update_bitmask_merge3d_v1(struct dpu_hw_ctl *ctx,
		unsigned blk, bool enable)
{
	if (!ctx)
		return -EINVAL;

	if (enable)
		ctx->pending_merge_3d_flush_mask |= BIT(blk);

	if (ctx->pending_merge_3d_flush_mask)
		...
		unsigned long cap)
{
	if (cap & BIT(DPU_CTL_ACTIVE_CFG)) {
		...
		ops->update_bitmask_merge3d =
			dpu_hw_ctl_update_bitmask_merge3d_v1;
	} else {
