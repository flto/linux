	     ...
	     bool is_dual_dsi, bool is_dp, bool is_dsc)
{
	...
	/* XXX */
	if (!is_dp) {
		dpu_write(dpu, INTF_PROG_FETCH_START(i), is_dsc ? 0x1A0CC1 : 0x1B2731);
		dpu_write(dpu, INTF_CONFIG(i), 0x80800000);
...
set_dsc_config(struct dpu_kms *dpu, u32 i, struct drm_dsc_config *dsc)
{
	...
	dpu_write(dpu, DSC_CHUNK_SIZE(i), 0x021C0000);
	dpu_write(dpu, DSC_DELAY(i), 0x020E0200);
	dpu_write(dpu, DSC_SCALE_INITIAL(i), 0x00000020);
	dpu_write(dpu, DSC_SCALE_DEC_INTERVAL(i), 0x00000007);
	dpu_write(dpu, DSC_SCALE_INC_INTERVAL(i), 0x000000BB);
	dpu_write(dpu, DSC_FIRST_LINE_BPG_OFFSET(i), 0x0000000C);
	dpu_write(dpu, DSC_BPG_OFFSET(i), 0x0DB70CB7);
	dpu_write(dpu, DSC_DSC_OFFSET(i), 0x180010F0);
...
static void
dpu_crtc_disable(struct drm_crtc *crtc, struct drm_crtc_state *state)
{
	drm_crtc_vblank_off(crtc);
}

static void
dpu_crtc_enable(struct drm_crtc *crtc, struct drm_crtc_state *state)
{
	drm_crtc_vblank_on(crtc);
}

static int
dpu_crtc_atomic_check(struct drm_crtc *crtc, struct drm_crtc_state *state)
{
...
static void
dpu_crtc_atomic_begin(struct drm_crtc *crtc, struct drm_crtc_state *old_state)
{
	/* nothing */
}
...
static void
dpu_crtc_atomic_flush(struct drm_crtc *crtc, struct drm_crtc_state *old_state)
{
	...
	struct mixercfg mixer_cfg[2] = {};
	struct drm_rect mixer_rect[2];
	u32 intf_active = 0;
	...
	for (i = c->lm; i < c->lm + num_mixers; i++) {
		u32 cfg_index = num_mixercfg > 1 ? (i >= c->lm + num_mixers/2) : 0;
