...
void dp_aux_isr(struct msm_dp *dp)
{
	...
	isr = readl_relaxed(dp->base + REG_DP_INTR_STATUS);
	isr &= ~DP_INTERRUPT_STATUS1_MASK;
	intr_ack = (isr & DP_INTERRUPT_STATUS1)
			<< DP_INTERRUPT_STATUS_ACK_SHIFT;
	writel(intr_ack | DP_INTERRUPT_STATUS1_MASK,
	       dp->base + REG_DP_INTR_STATUS);
